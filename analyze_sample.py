...
def check_sfr():
    ...
    ha_snr_mask = CC.catalog['HALPHA_AMP'] * np.sqrt(CC.catalog['HALPHA_AMP_IVAR']) > 3
    hb_snr_mask = CC.catalog['HBETA_AMP'] * np.sqrt(CC.catalog['HBETA_AMP_IVAR']) > 3
...
def compare_sfr():
    ...
    **ADD MORE DETAIL**
    ...
    cigale_sfr = np.array(CC.catalog['SFR_CIGALE'])
    halpha_sfr = np.array(CC.catalog['SFR_HALPHA'])
    stellar_mass = np.array(CC.catalog['MSTAR_CIGALE'])
    gr_color = calc_color()
    #print(gr_color)
    ...
    rem_bad = np.invert(np.isnan(halpha_sfr - cigale_sfr))
    mean_diff = np.average((halpha_sfr - cigale_sfr)[rem_bad])
    err_diff = np.std((halpha_sfr - cigale_sfr)[rem_bad])
    #stellar_mass = stellar_mass[rem_bad]
...
def sfr_diagnostic():
    ...
    mask = generate_combined_mask(BGS_MASK, SFR_MASK)
...
def sfr_ms():
    ...
    mask = generate_combined_mask(BGS_MASK, SFR_MASK, MSTAR_MASK)
    ...
    print(sum(np.array(mstar < 1, dtype=bool)))

    """
    plt.scatter(mstar[~agn_mask], sfr[~agn_mask], alpha = 0.03, marker='+', label='Other')
    plt.scatter(mstar[agn_mask], sfr[agn_mask], alpha = 0.03, marker = '+', label='AGN')
...
def sfr_spread_plots():
    ...
    #plt.scatter(mstar[~agn_mask], distance[~agn_mask], marker='+', alpha=0.05, label='Other')
    #plt.scatter(mstar[agn_mask], distance[agn_mask], marker='+', alpha=0.05, label='AGN')
    ...
    #plt.scatter(np.log10(ne), distance, marker='+', alpha=0.05, label='AGN')
    plt.xlim(0, 7)
    plt.ylim(-3, 2.5)
    ...
    #plt.scatter(A_ha, distance, marker='+', alpha=0.05, label='AGN')
    plt.xlim(-1, 3)
    plt.ylim(-2, 2)
    plt.xlabel(r'A(H$\alpha$)')
    plt.ylabel(r'$\Delta$ SFR [$\log{M_\odot/yr}$]')
...
def sfr_mstar_plots():
    ...
    ne_sfr = np.log10(ne)[sfr_mask]
    ne_mstar = np.log10(ne)[mstar_mask]
    sfr_calc = sfr[sfr_mask]
    mstar_calc = mstar[mstar_mask]
    ...
    b = 0.1
    for i in np.arange(8, 11.5, b):
        p25, p50, p75 = np.percentile(ne_mstar[generate_combined_mask(mstar_calc >= i, mstar_calc < i + b)], (25, 50, 75))
        ne_25.append(p25)
        ne_50.append(p50)
        ne_75.append(p75)
        mrange.append(i + b * 0.5)

    plt.scatter(mstar_calc, ne_mstar, marker='+', alpha=0.05, color='tab:orange')
    plt.plot(mrange, ne_25, color='tab:blue', linestyle='dashed')
    plt.plot(mrange, ne_50, color='tab:blue')
    plt.plot(mrange, ne_75, color='tab:blue', linestyle='dashed')
    plt.xlabel(r'$M_\star [\log{M_\odot}]$')
    ...
    plt.hist2d(mstar_calc, ne_mstar, bins=400)
    plt.plot(mrange, ne_25, color='r', linestyle='dashed')
    plt.plot(mrange, ne_50, color='r')
    plt.plot(mrange, ne_75, color='r', linestyle='dashed')
    plt.xlabel(r'$M_\star [\log{M_\odot}]$')
    plt.ylabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$")
    plt.xlim(8, 11.5)
    plt.ylim(2.2, 4.3)
    ...
    plt.hist2d(mstar_calc, ne_mstar, bins=400, norm=mpl.colors.LogNorm())
    plt.plot(mrange, ne_25, color='r', linestyle='dashed')
    plt.plot(mrange, ne_50, color='r')
    plt.plot(mrange, ne_75, color='r', linestyle='dashed')
    plt.xlabel(r'$M_\star [\log{M_\odot}]$')
    ...
    for i in np.arange(-2, 2, b):
        p25, p50, p75 = np.percentile(ne_sfr[generate_combined_mask(sfr_calc >= i, sfr_calc < i + b)], (25, 50, 75))
        ne_25.append(p25)
        ne_50.append(p50)
        ne_75.append(p75)
        sfrrange.append(i + b * 0.5)

    plt.scatter(sfr_calc, ne_sfr, marker='+', alpha=0.05, color='tab:orange')
    ...
    plt.hist2d(sfr_calc, ne_sfr, bins=200)
    ...
    plt.hist2d(sfr_calc, ne_sfr, bins=200, norm=mpl.colors.LogNorm())
    plt.plot(sfrrange, ne_25, color='r', linestyle='dashed')
    plt.plot(sfrrange, ne_50, color='r')
    plt.plot(sfrrange, ne_75, color='r', linestyle='dashed')
    plt.xlabel(r'SFR [$\log{M_\odot/yr}$]')
...
def sfr_mstar_vs_ne_plots():

    mass = CC.catalog['MSTAR_CIGALE'][BGS_MASK]  # bgs length
    sfr = CC.catalog['SFR_HALPHA'][BGS_MASK]  # bgs length
    ne, ne_mask = generate_ne_split_snr()  # these are both bgs length

    snr_mask = generate_combined_mask(SFR_BGS_MASK, ne_mask)
    ...
    b = 0.1
    for i in np.arange(8, 11.5, b):
        bin = generate_combined_mask(mstar >= i, mstar < i + b)
    ...
    plt.scatter(mstar, ne, marker='+', alpha=0.05, color='tab:orange')
    plt.plot(mrange, ne_25, color='tab:blue', linestyle='dashed')
    plt.plot(mrange, ne_50, color='tab:blue')
    plt.plot(mrange, ne_75, color='tab:blue', linestyle='dashed')
    plt.xlabel(r'$M_\star [\log{M_\odot}]$')
    ...
    plt.xlim(8, 11.5)
    plt.ylim(1, 3)
    plt.savefig('figures/ne_plots/ne_mass_scatter.png')
    plt.show()

    plt.hist2d(mstar, ne, bins=200)
    plt.plot(mrange, ne_25, color='r', linestyle='dashed')
    plt.plot(mrange, ne_50, color='r')
    plt.plot(mrange, ne_75, color='r', linestyle='dashed')
    plt.xlabel(r'$M_\star [\log{M_\odot}]$')
    ...
    plt.xlim(8, 11.5)
    plt.ylim(1, 3)
    plt.savefig('figures/ne_plots/ne_mass_2dhist.png')
    plt.show()

    plt.hist2d(mstar, ne, bins=200, norm=mpl.colors.LogNorm())
    plt.plot(mrange, ne_25, color='r', linestyle='dashed')
    plt.plot(mrange, ne_50, color='r')
    plt.plot(mrange, ne_75, color='r', linestyle='dashed')
    plt.xlabel(r'$M_\star [\log{M_\odot}]$')
    ...
    for i in np.arange(-2, 1.9, b):
        bin = generate_combined_mask(sfr >= i, sfr < i + b)
        p25, p50, p75 = np.percentile(ne[bin], (25, 50, 75))
        ne_25.append(p25)
        ne_50.append(p50)
        ne_75.append(p75)
    ...
    plt.scatter(sfr, ne, marker='+', alpha=0.05, color='tab:orange')
    plt.plot(sfrrange, ne_25, color='tab:blue', linestyle='dashed')
    plt.plot(sfrrange, ne_50, color='tab:blue')
    plt.plot(sfrrange, ne_75, color='tab:blue', linestyle='dashed')
    plt.xlabel(r'SFR [$\log{M_\odot/yr}$]')
...
def plot_ne_distribution_OLD():
    ...
    oii_snr = oii_snr[BGS_MASK]
    sii_snr = sii_snr[BGS_MASK]
    combined_ratio_mask = generate_combined_mask(oii_snr, sii_snr)

    log_ne_oii = np.log10(ne_oii[combined_ratio_mask])
    log_ne_sii = np.log10(ne_sii[combined_ratio_mask])
    ...
    plt.hist(oii_ratio, bins=200)
    plt.title("[OII] line ratio distribution", size=axis_size)
    plt.xlabel(r'$F_{\lambda 3726} / F_{\lambda 3729}$', size=axis_size)
    plt.xlim(0.25, 1.5)
    plt.show()

    plt.hist(sii_ratio, bins=200)
    plt.title("[SII] line ratio distribution", size=axis_size)
    plt.xlabel(r'$F_{\lambda 3731} / F_{\lambda 3716}$', size=axis_size)
    plt.xlim(0.25, 1.5)
    plt.show()

    nan_mask = generate_combined_mask(np.isfinite(log_ne_sii), np.isfinite(log_ne_oii))

    plt.hist2d(log_ne_sii[nan_mask], log_ne_oii[nan_mask], bins=100)
    plt.xlabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$ from [SII]")
    plt.ylabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$ from [OII]")
    plt.xlim(1, 3)
    plt.ylim(1, 3)
    plt.show()

    print(np.median(log_ne_oii[nan_mask]))
    print(np.median(log_ne_sii[nan_mask]))

    plt.hist2d(log_ne_sii[nan_mask], log_ne_oii[nan_mask], bins=100, norm=mpl.colors.LogNorm())
    plt.plot(np.array([0, 10]), np.median(log_ne_oii[nan_mask])*np.ones(2), color='r')
    plt.plot(np.median(log_ne_sii[nan_mask])*np.ones(2), np.array([0, 10]),  color='r')
    plt.xlabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$ from [SII]")
    plt.ylabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$ from [OII]")
    plt.xlim(.7, 3.5)
    plt.ylim(.7, 3.5)
    ...
    ne = np.zeros(len(oii_snr))

    for i in range(len(oii_snr)):
        if oii_snr[i] == True and sii_snr[i] == False:
            ne[i] = np.log10(ne_oii[i])
        elif oii_snr[i] == False and sii_snr[i] == True:
            ne[i] = np.log10(ne_sii[i])
        elif oii_snr[i] == True and sii_snr[i] == True:
            ne[i] = np.log10((ne_sii[i] + ne_oii[i]) / 2)
        elif oii_snr[i] == False and sii_snr[i] == False:
            ne[i] = -1

    ne_mask = generate_combined_mask(ne > 0, ~np.isnan(ne))

    #nan_mask = generate_combined_mask(~np.isnan(mass), ~np.isnan(sfr), ~np.isnan(ne))

    plt.hist(ne[ne_mask], bins=100)
    plt.xlim(0, 5)
    plt.title(f"Electron density distribution across BGS ({sum(~np.isnan(ne[ne_mask]))} galaxies)")
...
def plot_only_ne_distribution():
    ne, ne_mask = generate_ne_split_snr()
...
def plot_ne_distribution(snr_lim=5):
    ...
    ne_oii = np.log10(CC.catalog['NE_OII'])
    ne_sii = np.log10(CC.catalog['NE_SII'])

    ne_oii = ne_oii.filled(-999)
    ne_sii = ne_sii.filled(-999)

    finite_mask = generate_combined_mask(ne_oii > -998, ne_sii > -998, oii_snr, sii_snr)

    oii_med = np.median(ne_oii[finite_mask])
    sii_med = np.median(ne_sii[finite_mask])

    plt.hist2d(ne_sii[finite_mask], ne_oii[finite_mask], bins=100, norm=mpl.colors.LogNorm())
    plt.plot(np.array([0, 10]), np.median(ne_oii[finite_mask])*np.ones(2), color='r')
    plt.plot(np.median(ne_sii[finite_mask])*np.ones(2), np.array([0, 10]),  color='r')
    plt.title(f'Joint SNR $>$ {snr_lim} ({len(ne_sii[finite_mask])})')
    plt.xlabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$ from [SII]")
    plt.ylabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$ from [OII]")
    plt.xlim(0, 3.5)
    plt.ylim(0, 3.5)
    plt.text(0.04, 3.2, r'[OII] median {:.2f}'.format(oii_med) + '\n[SII] median {:.2f}'.format(sii_med))
    plt.savefig(f'figures/ne_plots/ne_comparison_snr{snr_lim}.png')
    plt.show()
    ...
    plt.hist(ne[ne_mask], bins=300)
    plt.xlim(0, 3.5)
    plt.title(f"Electron density distribution across BGS galaxies ({sum(ne_mask)})")
    plt.xlabel(r"$\log{n_e} ~[\mathrm{cm}^{-3}]$")
    plt.savefig(f'figures/ne_plots/ne_distribution.png')
    plt.show()
...
def generate_ne_split_snr(snr_lim=5):
...
def ne_histogram_binned():
    ...
    for m in masses:
        mll = m
        mul = m + mgap
        mcount = len(np.where(generate_combined_mask(mstar >= mll, mstar < mul) == True)[0])
        ...
        for i, (d, ax) in enumerate(zip(dists, axs)):
            ...
            mask = generate_combined_mask(mstar >= mll, mstar < mul, distance >= dll, distance < dul)
        ...
        plt.savefig(f'figures/ne_plots/ne_delta_sfr_mass_{m}_{m+mgap}.png')
        plt.show()
    ...
    for m in masses:
        ...
        for i, (d, ax) in enumerate(zip(sfrs, axs)):
            ...
            dul = d + sfrgap
            # Create bin for particular plot
            mask = generate_combined_mask(mstar >= mll, mstar < mul, sfr >= dll, sfr < dul)
            data = ne[mask]
            ax.set_xlim(xmin, xmax)
            if data.size > 0:
                bins = np.arange(min(data), max(data) + bin_width, bin_width)
        ...
        plt.savefig(f'figures/ne_plots/ne_sfr_mass_{m}_{m + mgap}.png')
        plt.show()


def sfr_sd():
    sfrsd = CC.catalog['SFR_SD'][BGS_MASK]
    sfrsd = sfrsd[SFR_BGS_MASK]
    #print(len(sfrsd[sfrsd<-1]))
    #sfrsd = sfrsd[sfrsd > -1]

    plt.hist(sfrsd, bins=150)
...
def run_analysis_scripts():
    global CC
    global BGS_MASK # This is catalog length
    global SFR_MASK # This is catalog length
    global MSTAR_MASK # This is catalog length
    global SFR_BGS_MASK
    global AGN_MASK  # This is BGS length
    CC = CustomCatalog()
    BGS_MASK = master_mask()
    SFR_MASK = hydrogen_snr_mask(snr_lim=5)
    SFR_BGS_MASK = hydrogen_snr_bgs_mask(snr_lim=5)
    MSTAR_MASK = mass_mask()

    #ah = np.array

    print(sum(np.array(CC.catalog["A_HALPHA"] < 0, dtype=bool)))
    print(sum(np.array(CC.catalog["A_HALPHA"][BGS_MASK] < 0, dtype=bool)))
    ...
    #identify_strange_a()
    #compare_sfr_snr()
    #quick_ebv_comparison()
    #plot_color_excess_vs_sfr_comparison()
    #check_color_ratio()
    #sfr_sd()
    ...
    #plot_ne_distribution(snr_lim=20)
    #sfr_mstar_vs_ne_plots()
    #sfr_mass_ne_colorplot()
    #sfr_mass_ne_colorplot_largebin()
    #ne_histogram_binned()
    #ne_vs_redshift()
